...
% muti_element_nodes
[element_nodes, node_coordinates, number_elements, number_nodes] = mesh_Q4([0, 0; 2, 0.5; 2, 1; 0, 1;], 16, 10);
...
for index = 1 : 10
    per = index * 34 + 1;
    prescribed_dof = [prescribed_dof, per, per + 1 ];
end
...
% force vector
force = zeros(G_dof, 1);
for index = 344 : 2 : length(force) - 2
    force(index) = - 2.5;
end
force(342) = - 1.25;
