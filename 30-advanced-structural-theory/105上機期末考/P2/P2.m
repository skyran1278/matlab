...
%       Due: 12/22/206
%
%      1. Complete the main program FRAME16
...
%			          ab=10m and bc=4m and settlement=10 mm. 
%            b. ITP=2: example 5.12 (page 127) 
%            c. ITP=3: example 5.3 (page 102) considering that a uniformly
%                      distributed load of 30 kN/m is applied downward 
%                      on member bc and 20 kN/m on member cd. 
...
%                       has a raise of 50 degrees. Assume that the coefficient of
...
%                       Material/Section: E=8000 N/cm^2, A=12 cm^2, I_1=20 cm^4, 
%                                         I_2=6 cm^4, J=18 cm^4.
%                       Boundary conditions: 1-fixed 2-free with the applied loads
%                         (global dir) of F_Y2=1000 N and F_Z2=2000 N 
...
[ELFOR,C,M,V] = FORCE(NDE,NBC,COOR,VECTY,IDBC,ITP,NCO,PROP,SECT,LM,DELTA,IFORCE,FEF);
% 
...
OUTPUT(IWRITE,TITLE,FILENAME,FTYPE,FUNIT,LUNIT,startTime,endTime,...
    ...
    VECTY,EXLD,IFORCE,FEF,DELTA,ELFOR,NSBAND,C,M,V);
 fclose(IWRITE);
% 
...
function [ELFOR,C,M,V] = FORCE(NDE,NBC,COOR,VECTY,IDBC,ITP,NCO,PROP,SECT,LM,DELTA,IFORCE,FEF)
%..........................................................................
...
for IB = 1:NBC
    ...
    %%%%volumn
        Volumn(1,IB)=SECT(1,IDBC(4,IB))*RL;
    %%%%mass
        Mass(1,IB)=PROP(4,IDBC(3,IB))*SECT(1,IDBC(4,IB))*RL;
    %%%%cost
        Cost(1,IB)=PROP(4,IDBC(3,IB))*PROP(3,IDBC(3,IB))*SECT(1,IDBC(4,IB))*RL;
...
%%%%cost
fprintf(IWRITE,'Total Volumn : %f m^3\r\n',V);
